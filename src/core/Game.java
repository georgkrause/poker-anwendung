...
//							 erhöht den Einsatz
		...
		if (activePlayerNumber == 1) {
			...
			if (this.tableCards[4].isVisible()) {
				...
				for (int i = 0; i < 4; i++) {
					if (!activePlayers[i].isFolded()) {
						int[] sortedcards = this.activePlayers[i].sortWorth(7, tableCards,
								activePlayers[i].getCards());
						int fiveColor = this.activePlayers[i].fiveColor(7, tableCards,
								activePlayers[i].getCards());
						this.activePlayers[i].pairWorth = this.activePlayers[i].sameWorth(7,
								sortedcards);
						int followfive = this.activePlayers[i].followFive(7, sortedcards);
						if (followfive != 0)
						...
						activePlayers[i].handWorth = activePlayers[i].handWorth(sortedcards,
